...
msckf::VioManagerOptions make_euroc_options() {
    ...
    // Official's euroc_mav config runs 50 SLAM landmarks and gets 0.133 m; DOD
    // cannot yet, so this stays off. Measured on EuRoC MH_01, all with the
    // feature-pointer bug fixed (see msckf/vio_manager.cpp):
    //
    //     SLAM off                     ATE 0.3947 m
    //     SLAM on, one joint update    ATE 1.2506 m
    //     SLAM on, official batching   ATE 1.1296 m
    //
    // The landmark LIFECYCLE is now correct -- promotion, retirement and the
    // id range were all broken and are fixed, and the subsystem holds its full
    // 50 landmarks with ~37 updates/frame. An earlier 0.2632 m measurement
    // looked encouraging but was an artifact: that build handed update_slam
    // stale pointers into a compacted array, so SLAM was barely running.
    //
    // Every function in the SLAM path has since been checked against official
    // and corrected where it differed -- the Jacobian now goes through the
    // bit-verified get_feature_jacobian_mixed instead of a second hand-written
    // implementation, StateHelper::initialize uses official's in-place Givens
    // rather than a HouseholderQR with an explicit Q, and the invented
    // symmetrisation/jitter/LDLT in both are gone. None of it moved the
    // number: SLAM still costs 3x.
    //
    // What the instrumentation points at now is the landmarks themselves --
    // 69% of SLAM candidates fail triangulation at init (17475 of 25480), so
    // the ones that pass are likely marginal, and 37 weak landmarks per frame
    // drag the estimate. That is a feature-quality problem, the same one that
    // makes 46% of MSCKF features fail the condition-number test, not an
    // estimator-math problem.
    options.enable_slam = false;
...
// One observation replayed from official's dump.
struct InjectedObs {
    std::size_t featid;
    int cam;
    double u, v, un, vn;
...
// timestamp -> observations at that frame
static std::map<double, std::vector<InjectedObs>> g_tracks;

static bool load_tracks(const char* path) {
    std::FILE* fp = std::fopen(path, "r");
    if (!fp) {
        std::fprintf(stderr, "track injection: cannot open %s\n", path);
        return false;
    }
    double ts, u, v, un, vn;
    unsigned long fid, cam;
    long n = 0;
    // Hex floats in the dump, so the replayed measurement is bit-identical to
    // the one official's estimator consumed.
    while (std::fscanf(fp, "%lf %lu %lu %la %la %la %la", &ts, &fid, &cam, &u, &v, &un, &vn) == 7) {
        InjectedObs o;
        o.featid = (std::size_t)fid;
        o.cam = (int)cam;
        o.u = u; o.v = v; o.un = un; o.vn = vn;
        g_tracks[ts].push_back(o);
        ++n;
    }
    std::fclose(fp);
    std::fprintf(stderr, "track injection: %ld observations over %zu frames\n", n, g_tracks.size());
    return n > 0;
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::fprintf(stderr, "usage: %s BAG_PATH OUTPUT_PREFIX [max_seconds] [tracks_file]\n", argv[0]);
        return 2;
    }
    ...
    const char* tracks_path = (argc > 4) ? argv[4] : "/tmp/ov_tracks.txt";
    if (!load_tracks(tracks_path)) return 6;
    ...
    for (const rosbag::MessageInstance& message : view) {
        ...
        // TRACK INJECTION: instead of running DOD's tracker, replay the exact
        // observations official produced for this frame. Everything downstream
        // -- triangulation, MSCKF, SLAM -- then sees identical measurements to
        // official's, so any remaining trajectory difference is the estimator
        // and any difference that DISAPPEARS was the frontend.
        int observation_count = 0;
        {
            auto it = g_tracks.lower_bound(camera_timestamp - 1e-4);
            if (it != g_tracks.end() && std::abs(it->first - camera_timestamp) < 1e-4) {
                for (const InjectedObs& o : it->second) {
                    if (observation_count >= core::TRACKER_MAX_FEATURES * 2) break;
                    core::Feature& f = observations[observation_count++];
                    f = core::Feature{};
                    f.featid = (int)o.featid;
                    f.num_measurements = 1;
                    f.measurements[0].cam_id = o.cam;
                    f.measurements[0].timestamp = camera_timestamp;
                    f.measurements[0].uv = Eigen::Vector2d(o.u, o.v);
                    f.measurements[0].uv_norm = Eigen::Vector2d(o.un, o.vn);
                }
            }
        }
        (void)left_cv; (void)right_cv; (void)tracker;
