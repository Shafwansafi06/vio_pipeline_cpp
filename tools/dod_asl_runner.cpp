// ROS-free EuRoC runner: reads the dataset's own ASL folder layout (produced by
// tools/bag_to_asl.py) instead of a rosbag, so the full pipeline can be run and
// scored on a machine with no ROS install. Configuration comes from the shared
// tools/euroc_options.hpp, so this and ros/vio_rosbag_runner_euroc.cpp cannot
// drift apart.
//
