...
used by OpenVINS, built around fixed-capacity, allocation-free per-frame
state and a
...
public sequences (EuRoC MH/V1, KAIST circle/infinity), DODVins
...
99th percentile DODVins's frame latency advantage
...
independently measured effect of every accepted layout and algorithmic
change (Table~\ref{tab:ablation}, each verified bit-identical on all ten
trajectories before and after), an
...
numbers). Regenerated from the same fresh 2026-08-30 runs as
Table~\ref{tab:ate}. KAIST circle's blue (ours) trace visibly loops near
the origin before settling onto the ground-truth circle -- the
initialization-latency transient discussed in Section~\ref{sec:limitations}.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/tradeoff.png}
...
features per update as OpenVINS was tested by capping DODVins
...
actually is: on both sequences DODVins initializes 1.8--2.8\,s
...
the first 5\,s of MH\_02 is 0.2133\,m for DODVins against
