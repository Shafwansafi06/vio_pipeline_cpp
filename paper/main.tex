...
\title{DODVins: A Data-Oriented, Allocation-Free MSCKF\\
Visual-Inertial Odometry Pipeline with Measured Parity to OpenVINS}
...
used by OpenVINS. The filter equations are unchanged; what differs is the
engineering: fixed-capacity, allocation-free per-frame state, no virtual
dispatch, and a feature database that separates search keys from payloads.
...
flat 184--199\,MB memory footprint against OpenVINS's variable one. The
...
estimator's mathematics. Mike Acton and Richard Fabian's data-oriented
design (DOD) literature \cite{fabian2018dod} argues that on such paths, the
layout of data in memory --- not the algorithm expressed over it --- is
usually the dominant cost, and that object-oriented abstractions frequently
hide layout decisions a profiler would reject. This paper tests that
argument on a production-shaped VIO pipeline rather than a microbenchmark:
we reimplement OpenVINS's MSCKF pipeline in C++ under a small set of house
rules --- no per-frame heap allocation, no virtual dispatch, all state in
fixed-capacity tables, one transform per table --- and measure the result
against the original on identical data through an identical transport.
...
DODVins-attributable churn, the largest being three heap temporaries
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{figs/latency_cdf.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{figs/tradeoff.png}
...
OpenVINS's on every sequence at statistically equal accuracy; the
...
profiled reason (row 4). On this codebase, plausible causal narratives
about where time goes have been wrong more often than right; every change
...
accumulated global offset. Table~\ref{tab:rpe} shows DODVins's
...
DODVins initializes 1.8--2.8\,s \emph{earlier} than OpenVINS and starts
from a measurably worse initial state (first-5\,s RMSE on MH\_02: 0.2133\,m
vs.\ 0.1188\,m) despite near-identical drift thereafter. MH\_02's overall
ATE gap (Table~\ref{tab:ate}) is attributable to this initialization
...
or not a sequence needs it. Per-sequence latency CDFs for both estimators
(x86 counterpart: Figure~\ref{fig:cdf}) show the same direction; the margin
does not transfer in full.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{figs/tumvi_all_rooms.png}
...
relative-pose-error analysis shows DODVins's steady-state
...
For a practitioner the guidance this benchmark supports is narrow but
concrete. On desktop-class x86, DODVins delivers the same MSCKF
trajectories as OpenVINS for roughly half the wall-clock, a quarter to a
third less processor time, a frame-latency tail that stays under the
sensor's frame budget where the reference's does not, and a flat memory
footprint --- at the price of a larger, pre-allocated working set and no
monocular support. On a Jetson-class embedded SoC the direction of every
one of those statements is unchanged; only the margin narrows. Teams whose
estimator must share the compute budget with control and navigation get
the freed share without changing a filter equation; teams relying on loop
closure or tight memory ceilings should weigh the trade-offs recorded in
Section~\ref{sec:limitations} before choosing.
