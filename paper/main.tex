...
\author{
\IEEEauthorblockN{Shafwan Safi, Nikhil Vashista}
...
the SchurVINS path. Benchmarked head-to-head against the authors' own released
\texttt{ov\_SchurVINS} fork, our primary pipeline is 1.97$\times$ faster
end-to-end on identical data (Table~\ref{tab:ablation}, row 9).
...
were taken from the same build and the same run. Benchmarked separately
against the authors' own released \texttt{ov\_SchurVINS} fork on MH\_01
(static initialization, bag\_start~40, matching its own launch protocol),
the reimplementation is 1.97$\times$ faster than \texttt{ov\_SchurVINS} and
2.16$\times$ faster than stock OpenVINS on the same protocol.
...
9 & vs.\ authors' \texttt{ov\_SchurVINS} (own protocol) & 1.97$\times$ faster \\
\bottomrule
\end{tabular}
\end{table*}
...
\textbf{SchurVINS support is built but unused.} The reimplementation
includes a data-oriented SchurVINS update path (Section~\ref{sec:rules}'s
constraints applied to \cite{fan2024schurvins}'s algorithm), gated behind a
runtime flag that defaults off. No result in this paper uses it; it is
reported (Table~\ref{tab:ablation}, row 9) purely as a second, independent
speed comparison against a second reference implementation on the same
data.
