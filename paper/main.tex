...
and heap-allocated per-feature and per-measurement objects. This is not a
criticism of OpenVINS's engineering; it is an observation about what that
design pattern costs on a per-frame hot path, independent of the
...
200\,Hz) would freeze permanently --- a silent initialization failure with
no counter to make it visible. An IMU-window selection routine silently
