#!/usr/bin/env python3
"""Overlay DOD/OpenVINS(/Python) trajectories against ground truth for a given
dataset prefix. Generalizes plot_circle_comparison.py to also cover
infinity.bag (DOD vs OpenVINS only, no Python v2 run for that dataset).

Usage: plot_trajectory_comparison.py PREFIX GT_PREFIX LABEL OUT_NAME [series...]
  series entries are "name:file_prefix", e.g. "DOD C++ (ours):dod_infinity"
"""
import csv
import sys
...
def aligned(file_prefix, gt_prefix):
    t, p = load_positions(RESULTS / f"{file_prefix}_estimate.csv")
    tt, tp = load_positions(RESULTS / f"{gt_prefix}_groundtruth.csv")
    ie, it = associate(t, tt)
    r, tr = rigid_align(p[ie], tp[it])
    return t[ie], (r @ p[ie].T).T + tr
...
def main():
    ...
    title = sys.argv[2]
    out_name = sys.argv[3]
    series_args = sys.argv[4:]

    truth_t, truth_p = load_positions(RESULTS / f"{gt_prefix}_groundtruth.csv")
    series = {}
    for arg in series_args:
        name, file_prefix = arg.split(":", 1)
        series[name] = aligned(file_prefix, gt_prefix)
    colors = {name: COLORS[i % len(COLORS)] for i, name in enumerate(series)}

    fig, axes = plt.subplots(2, 2, figsize=(13, 12))
    ax_full = axes[0, 0]
    ax_full.plot(truth_p[:, 0], truth_p[:, 1], color="black", linewidth=2, label="Ground truth")
    for name, (_, est) in series.items():
    ...
    for ax, (label, lo, hi) in zip([axes[0, 1], axes[1, 0], axes[1, 1]], segments):
        ...
        for name, (est_t, est) in series.items():
